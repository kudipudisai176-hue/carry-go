      ...
      <div className={`w-full max-w-md rounded-3xl border border-orange-100 bg-white/80 p-8 shadow-2xl backdrop-blur-xl transition-all duration-500 group/card ${role === 'traveller' ? 'hover:border-purple-500/40 hover:shadow-[0_0_40px_rgba(168,85,247,0.1)]' : 'hover:border-orange-500/40 hover:shadow-[0_0_40px_rgba(249,115,22,0.1)]'
        ...
        <div className="mb-8 text-center text-slate-800">
          <motion.div
            ...
            className={`mx-auto mb-4 flex h-14 w-14 cursor-pointer items-center justify-center rounded-2xl shadow-lg transition-all duration-300 ${role === 'traveller' ? 'bg-purple-600' : role === 'receiver' ? 'bg-indigo-600' : 'bg-orange-500'
              }`}
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="group space-y-2">
            <Label htmlFor="email" className="flex items-center gap-2 font-medium text-slate-700 transition-colors group-hover:text-orange-500">
          ...
          <div className="group space-y-2">
            <Label htmlFor="password" className="flex items-center gap-2 font-medium text-slate-700 transition-colors group-hover:text-orange-500">
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <Link
            to="/signup"
            className={`font-semibold underline-offset-4 decoration-2 hover:underline transition-colors ${role === 'traveller' ? 'text-purple-400 hover:text-purple-300' : 'text-orange-400 hover:text-orange-300'
              }`}
          >
            Sign Up
          </Link>
